//In HW context: G = [u]; u = [v, weight] for edge u -> v
const prims: (graph: number[][][]) => void = (graph: number[][][]) => {
    const list = [1];
    let numAdded = 1;
    while (list.length < graph.length) {
        let lowestEdge = [-1, -1, 99999];
        for (let j = 0; j < list.length; j++) {
            let u = list[j] - 1; //From vertex
            for (let edge of graph[u]) {
                if (!list.includes(edge[0]) && lowestEdge[2] > edge[1]) {
                    lowestEdge = [u+1, edge[0], edge[1]];
                }
            }
        }

        if (lowestEdge[1] !== -1) {
            list.push(lowestEdge[1]);
            console.log(`Edge ${numAdded} is (${lowestEdge[0]}, ${lowestEdge[1]})`);
            numAdded++;
        }
    }
}
